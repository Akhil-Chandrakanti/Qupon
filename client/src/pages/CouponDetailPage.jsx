...
export default function CouponDetailPage() {
  ...
  const handlePaymentComplete = async (payerRef) => {
    setBuyLoading(true);
    setError('');
    try {
      const res = await api.post(`/coupons/${id}/buy`, { upiId: payerRef });
      setSuccess(res.data);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 animate-fadeIn">
        ...
        <div className="md:col-span-3 space-y-6">
          ...
          <div className="card overflow-hidden">
            <div className="bg-gradient-to-br from-brand-600 to-brand-700 p-8 text-white relative overflow-hidden">
              {getUploadedImageUrl(coupon.image) && (
                <>
                  <img src={getUploadedImageUrl(coupon.image)} alt={coupon.title}
              ...
              <div className="relative">
                <div className="flex items-center gap-3 mb-4">
                  <span className="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-2">
                    {!logoFailed && getBrandLogoUrl(coupon.brand) && (
                      <span className="w-5 h-5 bg-white rounded-md flex items-center justify-center overflow-hidden shrink-0">
                        <img src={getBrandLogoUrl(coupon.brand)} alt={coupon.brand} className="w-4 h-4 object-contain"
                ...
                <h1 className="text-2xl md:text-3xl font-black mb-3 leading-snug drop-shadow-sm">{coupon.title}</h1>
      ...
      {showBuyModal && (
        <PaymentGatewayModal
          ...
          onCancel={() => { setShowBuyModal(false); setError(''); }}
        />
      )}
    </div>
  );
